...
| `design.md` §10.4 distribution-light core | **replace** the estimator with the selected candidate | `realdata/pwm_*{,_meCorrected}.{csv,json}` | **DELIVERED on real joint** (issue #11): genome-wide PWM decomposition, all 18,129 genes. me-removal via NTC control done → median share_T **0.019 (raw) → 0.081 (me-corrected)**; genes share_T>0.10 **40 → 7,674 (42%)** once the measurement-noise floor is removed. Top ENSG00000165953 share_T=0.908 |
