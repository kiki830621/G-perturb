...
About 440 words, roughly 2:55 at a normal speaking pace, inside the 3-minute limit. The bracketed headers are pacing markers, not spoken. Read it plainly, the way you would explain the work to a colleague; there is no need to hit any word hard.

---

[0:00 · The problem]

Perturb-seq screens usually rank drug targets by the extent to which  perturbation shifts gene expression. But a large effect seen in a single guide, or a single donor, can be measurement noise rather than real biology, and a target picked on that basis may not reproduce.
